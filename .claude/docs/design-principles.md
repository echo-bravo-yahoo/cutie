...
- `Control extends Step` (`src/util/Control.ts:12`) — a step that decides what the chain does next rather than changing the message on its way through. The base class adds nothing; it exists so that flow control is a kind of its own, rather than a transform that has to declare `honorsTargeting = false` and then carry a `transformSingle` nothing calls. There are three: `control:return` (`src/controls/return.ts:16`) returns a `Returned` carrying the value to hand back and the keys to publish into the caller's stash; `control:branch` (`src/controls/branch.ts:17`) calls `Task.invoke` on the task its `task:` names and carries on with what that task decided; and `control:stop` (`src/controls/stop.ts:15`) returns `HALT`. The latter two share an optional `when` predicate, compiled once at registration by `compilePredicate` (`src/util/javascript.ts:65`) and read for JavaScript's own truthiness. `when` means one thing in both — when this holds, do what the module is named for — which is what naming the module `stop` rather than `filter` buys: a `filter` would have read "continue when this holds", running its predicate backwards from a branch's.
...
The rule below is not implemented. This records where the design currently is, what was decided versus what merely fell out, and what a change would have to settle.
...
- **The write-back.** `Task.invoke` publishes the keys `control:return` names into `caller.stash` by reference. That is sound for a rescue, which is synchronously nested inside the message being rescued and finishes before it continues, and sound for a `control:branch` for the same reason: it awaits `invoke` from inside its own `doHandleMessage`, so the caller has not moved on. It is not sound for `output:event` or the log fan-out: neither awaits the receiving chain, so the originating chain has moved on by the time the callee returns, and the write lands in a stash nobody reads again - or one a later step has since rewritten. Any inheritance beyond the two awaited calls needs the hand-off to be one-way.
