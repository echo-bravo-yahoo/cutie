...
- `control:branch` runs another task from inside this one and then carries on. The target is an ordinary task named by `task:`, and it decides what comes back exactly as a rescue does: end it at a `control:return` to replace the message, or let it fall off its own end to leave the message the branch was handed. `cutie validate` refuses a branch naming a task the config does not declare, and one that leads back to the task it branches from -- rescues and branches share one graph, so a loop closed by one of each is caught too.
- `control:stop` ends the chain, so the steps after it never run. Dropping a message used to mean making a step throw, which filed an `error` line for an entirely ordinary condition; a stopped message is consumed rather than failed, the way an incomplete `transform:accumulate` batch already was.
- Both take an optional `when`, a JavaScript function body read for truthiness, compiled once when the task registers. It means the same thing in either module -- when this holds, do what the module is named for -- so a `control:branch` runs its task when `when` holds and a `control:stop` stops the chain when `when` holds. Omit it to do that every time. A predicate that throws is an ordinary step failure, reported under its own topic and routed by its `rescue`, so a typo'd path cannot read as "condition not met".
- `cutie validate` warns about a `control:return` in a task that nothing invokes. The step is inert there, because the chain ends and the value it names has nowhere to go.
