...
function bitArrayToByte(bitArray, lsbFirst = true) {
  ...
  if (bitArray.length !== 8)
    console.log(
      `Bit array is ${bitArray.length} bits long, but it should be 8 bits long to convert to a byte!`,
    );
  let result = 0x00;
  for (let i = 0; i < 8; i++) {
    if (lsbFirst) {
    ...
    for (let j = 0; j < buckets.length; j++) {
      ...
      if (is(wave[i].duration, bucket.duration, bucket.tolerance)) {
        ...
        if (i % 5 === 0) {
          ...
          if (
            markers[currentLine].length === highGraphLines[currentLine].length
          )
            markers[currentLine] += " ";
        }

        if (wave[i].level) {
          highGraphLines[currentLine] += bucket.indicator
            ? bucket.indicator
            : j;
      ...
      if (fit.length > 1)
        throw new Error(
          `Pulse at index ${i} with duration ${wave[i].duration} fit multiple buckets!\n${JSON.stringify(fit)}`,
        );
    }
  }
...
function is(value, expected, tolerance = 0.33) {
...
function highWaveFromDuration(
  ...
  for (let index = 0; index < cycles; index++) {
    pulses.push({
      gpioOn: ledPin,
      gpioOff: 0,
      usDelay: Math.round(usDelay * dutyCycle),
    });
    pulses.push({
      gpioOn: 0,
      gpioOff: ledPin,
      usDelay: Math.round(usDelay * (1 - dutyCycle)),
    });
  }

  pulses.forEach(
    (pulse) => (generatedCSV += `${pulse.gpioOn ? 1 : 0},${pulse.usDelay}\n`),
  );
...
function validateComplement(a, b) {
  if (a.length !== b.length)
    throw new Error(`Cannot be complimentary (differing lengths).`);
  for (let i = 0; i < a.length; i++) {
    if (a[i] === b[i])
      throw new Error(
        `Not complimentary; at index ${i}, both bytes contain the value ${a[i]}.`,
      );
  }
}
...
function readBit(duration, highDuration = 1688, lowDuration = 563) {
  if (is(duration, highDuration)) {
    ...
    console.error(
      `Bit with duration ${duration} is not the expected size (${lowDuration} or ${highDuration}).`,
    );
