...
const readNECByte = (array, startIndex) =>
  readByte(array, startIndex, false, 1688, 563);
...
function necToBits({
  ...
  logger(
    `Address             : (phys) ${JSON.stringify(addressBits.map((bit) => (bit ? 1 : 0)))} (logical ${bitArrayToByte(addressBits)}, 0x${bitArrayToByte(addressBits).toString(16)})`,
  );
  logger(
    `Address (complement): (phys) ${JSON.stringify(extendedAddressBits.map((bit) => (bit ? 1 : 0)))} (logical ${bitArrayToByte(extendedAddressBits)}, 0x${bitArrayToByte(extendedAddressBits).toString(16)})`,
  );
  logger(
    `Command             : (phys) ${JSON.stringify(commandBits.map((bit) => (bit ? 1 : 0)))} (logical ${bitArrayToByte(commandBits)}, 0x${bitArrayToByte(commandBits).toString(16)})`,
  );
  logger(
    `Command (complement): (phys) ${JSON.stringify(extendedCommandBits.map((bit) => (bit ? 1 : 0)))} (logical ${bitArrayToByte(extendedCommandBits)}, 0x${bitArrayToByte(extendedCommandBits).toString(16)})`,
  );
...
function waveToNec(wave, validateAddress = false, validateCommand = true) {
  if (!wave[0] || wave[0].level !== 1 || is(wave[0], 9000))
    throw new Error(
      `First pulse should be high for 9000 uS.\n\nSegments: ${JSON.stringify(wave, null, 2)}`,
    );

  if (!wave[1] || wave[1].level !== 0 || is(wave[1], 4500))
    throw new Error(
      `Second pulse should be low for 4500 uS.\n\nSegments: ${JSON.stringify(wave, null, 2)}`,
    );
  ...
  wave.forEach((segment, index) => {
    ...
    if (
      ...
      index !== 1 &&
      segment.level === 1 &&
      !is(segment.duration, 563) &&
      !is(segment.duration, 281)
    )
      throw new Error(
        `Seperator at index ${index} is not the expected size (563); was actually ${segment.duration}.\n\nSegments: ${JSON.stringify(wave, null, 2)}`,
      );
  });

  let durations = wave
    .filter((segment) => segment.level === 0 && !is(segment, 4500))
  ...
  try {
    ...
    console.log(
      `Address             : ${JSON.stringify(address)} (${bitArrayToByte(address)}, 0x${bitArrayToByte(address).toString(16)})`,
    );
    console.log(
      `Address (complement): ${JSON.stringify(addressComplement)} (${bitArrayToByte(addressComplement)}, 0x${bitArrayToByte(addressComplement).toString(16)})`,
    );
    console.log(
      `Command             : ${JSON.stringify(command)} (${bitArrayToByte(command)}, 0x${bitArrayToByte(command).toString(16)})`,
    );
    console.log(
      `Command (complement): ${JSON.stringify(commandComplement)} (${bitArrayToByte(commandComplement)}, 0x${bitArrayToByte(commandComplement).toString(16)})`,
    );
...
async function transmitNECCommand(
  pigpio,
  address,
  command,
  extendedAddress,
  ...
  return new Promise((resolve, reject) => {
    pigpio.waveAddGeneric(
      necToWave(address, command, extendedAddress, extendedCommand),
    );
  ...
  setTimeout(
    () => {
      ...
      if (pulse.length === 67) {
        ...
        console.log(
          `Received an invalid NEC command with ${pulse.length} pulses. Starting over...`,
        );
        pulse = [];
      }
