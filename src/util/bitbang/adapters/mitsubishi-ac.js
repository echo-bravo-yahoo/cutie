...
const readMitsubishiByte = (array, startIndex) =>
  readByte(array, startIndex, false, 450, 1275);
...
function mitsubishiToWave(byteArray, simple) {
  console.log(
    "    mitsubishiToWave: " +
      byteArray
        .reduce((string, next) => (string += ` 0x${next.toString(16)}`), "")
        .trim(),
  );
  let low, high;
  if (simple) {
...
function waveToMitsubishiAC(wave) {
  if (!wave[0] || wave[0].level !== 1 || is(wave[0], 3500))
    throw new Error(
      `First pulse should be high for 3500 uS.\n\nSegments: ${JSON.stringify(wave, null, 2)}`,
    );

  if (!wave[1] || wave[1].level !== 0 || is(wave[1], 1700))
    throw new Error(
      `Second pulse should be low for 1700 uS.\n\nSegments: ${JSON.stringify(wave, null, 2)}`,
    );
  ...
  wave.forEach((segment, index) => {
    ...
    if (
      ...
      index !== 1 &&
      segment.level === 1 &&
      !is(segment.duration, 450) &&
      !is(segment.duration, 3620)
    )
      console.error(
        `Seperator at index ${index} is not the expected size (450); was actually ${segment.duration}.\n\nSegments: ${JSON.stringify(wave)}`,
      );
  ...
  let durations = wave
    .filter(
      (segment) =>
        segment.level === 0 &&
        !is(segment.duration, 1700, 0.16) &&
        !is(segment.duration, 17000),
    )
  ...
  console.log(
    `JSON formatted by mitsubishi.js: ${JSON.stringify(json, null, 2)}`,
  );
...
async function transmitMitsubishiCommand(pigpio, byteArray) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    console.log(
      `First 10 transmit durations: ${JSON.stringify(mitsubishiToWave(byteArray, true).slice(0, 10))}`,
    );
    console.log(
      graphToTerminal(mitsubishiToWave(byteArray, true), [
        { duration: 1275, tolerance: 0.14 },
        { duration: 450 },
        { duration: 3500 },
        { duration: 17000 },
        { duration: 1700, tolerance: 0.14 },
      ]),
    );
    writeGeneratedCSV();
  ...
  timeoutHandle = setTimeout(
    () => {
      ...
      if (pulse.length === pulseCount) {
        ...
        console.log(
          `Received an invalid Mitsubishi command with ${pulse.length} pulses. Starting over...`,
        );
        pulse = [];
      }
