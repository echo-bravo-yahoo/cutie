...
class ByteArray extends BitAware {
  ...
  static bitwiseReverse(byteArray, sourceName = "logical") {
    if (byteArray instanceof ByteArray) byteArray = byteArray[sourceName];
    return new ByteArray(
      byteArray.map((byte) => Byte.bitwiseReverse(byte, sourceName)),
    );
  }
  ...
  static bitwiseFlip(byteArray, sourceName = "logical") {
    if (byteArray instanceof ByteArray) byteArray = byteArray[sourceName];
    return new ByteArray(
      byteArray.map((byte) => Byte.bitwiseFlip(byte, sourceName)),
    );
  }
  ...
  equals(byteArray) {
    ...
    if (this.logical.length !== byteArray.length) return false;
    for (let i = 0; i < this.logical.length; i++) {
      if (!this.logical[i].equals(byteArray[i])) return false;
    }
    return true;
  }

  makeValidByte(byteish) {
    if (byteish instanceof Byte) return byteish;
    if (byteish instanceof BitArray) {
      if (byteish.logical.length !== 8)
        throw new Error(
          `Attempted to coerce BitArray (${byteish.toString(all)}) to a byte with width 8, but do not know what to do with the missing bits.`,
        );
  ...
  push(byteOrBytes) {
    ...
    if (byteOrBytes instanceof Array) {
      byteOrBytes.forEach((byteish) =>
        this.logical.push(this.makeValidByte(byteish)),
      );
    } else {
  ...
  sum(sourceName = "logical") {
    return this[sourceName].reduce(
      (sum, nextByte) => sum + nextByte.toNumber(),
      0,
    );
  }
  ...
  checksum(sourceName = "logical") {
    ...
    if (isNaN(checksum))
      throw new Error(
        `Computed checksum was NaN for byteArray ${this.toString("all")}.`,
      );
