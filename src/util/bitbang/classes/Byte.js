...
class Byte extends BitAware {
  constructor(numberOrBitArray, lsbFirst, flipLogic) {
    ...
    if (numberOrBitArray instanceof Byte) {
      ...
      throw new Error(
        `Cannot convert this (${numberOrBitArray}, ${JSON.stringify(numberOrBitArray)}, ${typeof numberOrBitArray}) into a Byte.`,
      );
    }
  }
  ...
  static bitwiseReverse(byte, sourceName = "logical") {
    return new Byte(
      BitArray.bitwiseReverse(Byte.toBitArray(byte, sourceName), sourceName),
    );
  }

  static bitwiseFlip(byte, sourceName = "logical") {
    return new Byte(
      BitArray.bitwiseFlip(Byte.toBitArray(byte, sourceName), sourceName),
    );
  }

  // mutates source, chainable
  bitwiseReverse(sourceName = "logical") {
    this[sourceName] = new BitArray(this[sourceName])
      .bitwiseReverse()
      .toNumber();
    return this;
  }
  ...
  toString(format = "dec", sourceName = "logical") {
    if (format === "dec" || format === "decimal") {
      ...
      return new BitArray(this[sourceName]).logical
        .map((bit) => (bit ? "1" : "0"))
        .join("");
    } else if (format === "all") {
