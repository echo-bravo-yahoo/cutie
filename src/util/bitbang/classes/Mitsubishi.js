...
class Mitsubishi {
  ...
  commandToBytes(command) {
    ...
    if (
      command.temperature ===
      clamp(command.temperature, this.minTemp, this.maxTemp)
    ) {
      // 16' C is represented by 0x0
      // but we add 32 for half-step degrees
      if (command.temperature === Math.floor(command.temperature) + 0.5) {
      ...
      command.temperature ===
      clamp(command.temperature, this.minTempF, this.maxTempF)
    ) {
      // otherwise, it's fahrenheit
      const celsius = this.celsiusIntToFahrenheitInt(command.temperature);
      if (
        celsius !== clamp(celsius, this.minTemp, this.maxTemp) &&
        celsius !== clamp(celsius, 32, 47)
      )
        console.error(
          `Temperature ${command.temperature} is out of bounds for either a Celsius temperature (${this.minTemp} - ${this.maxTemp}) or a Fahrenheit temperature (${this.minTempF} - ${this.maxTempF}).`,
        );
    ...
    bytes.push([new Byte(0x00), new Byte(0x00), new Byte(0x00)]);
  ...
  bytesToCommand(byteArray) {
    if (!(byteArray instanceof ByteArray)) byteArray = new ByteArray(byteArray);
    ...
    if (!header.equals(byteArray.slice(0, 5)))
      throw new Error(
        `Incorrect header for command. Was ${byteArray.slice(0, 5).toString("hex", "logical")}, should have been: ${header.toString("hex", "logical")}.`,
      );
    ...
    if (onByte.toNumber() === 0x20) {
      ...
      throw new Error(
        `Unrecognized mode ${onByte.toString("all")}. Should be either on (${new Byte(0x20).toString("all")}) or off (${new Byte(0x0).toString("all")}).`,
      );
    }
    ...
    if (temperatureByte.toNumber() > 15) {
      ...
      command.temperature = temperatureByte.toNumber() + 0.5;
    } else {
    ...
    if (secondMode !== command.mode)
      throw new Error(
        `First mode byte (${modeByte.toString("all")}) indicated a mode of ${command.mode}, but second mode byte (${secondModeByte.toString("all")}) indicated a mode of ${secondMode}. These bytes must agree on the AC mode.`,
      );
    ...
    if (fanByte.getBit(0)) {
      ...
      if (fanByte.slice(5, 8).toNumber() !== 0)
        throw new Error(
          `fanSpeed set to auto, but speed specified as ${fanByte.slice(5, 8).toString("bit", "logical")}. If fanSpeed is set to auto, speed bits must not be specified.`,
        );
    }
    ...
    if (!expectedFooter.equals(actualFooter))
      throw new Error(
        `Incorrect footer for command. Was ${actualFooter.toString("hex", "logical")}, should have been: ${expectedFooter.toString("hex", "logical")}.`,
      );
    ...
    if (actualChecksum.toNumber() !== expectedChecksum)
      console.error(
        `Incorrect checksum for command. Was ${new Byte(actualChecksum).toString("all")}, should have been: ${new Byte(expectedChecksum).toString("all")}.`,
      );
