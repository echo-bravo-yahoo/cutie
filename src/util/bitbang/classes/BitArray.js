...
class BitArray extends BitAware {
  constructor(numberOrBitArray, width = 8, lsbFirst, flipLogic) {
    ...
    this.logical = numberOrBitArray.length
      ? numberOrBitArray
      : BitArray.toBitArray(numberOrBitArray, width);
  }
  ...
  overwrite(startIndex, bitArray) {
    ...
    for (let i = 0; i < bitArray.length; i++) {
      if (startIndex + i >= this.logical.length)
        throw new Error(
          `Overwrite operation with BitArray of length ${bitArray.length} starting at index ${startIndex} extends beyond current BitArray's last index (${this.logical.length - 1}.`,
        );
