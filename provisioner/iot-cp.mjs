...
export async function createKeysAndRegisterThing() {
  ...
  promises.push(
    writeFile(
      resolve(
        `${config.provisioner.iot.awsCertFilePath}`,
        `../${config.hostname}-certificate.pem.crt`,
      ),
      keys.certificatePem,
      { mode: 0o600 },
    ),
  ...
  promises.push(
    writeFile(
      resolve(
        `${config.provisioner.iot.awsCertFilePath}`,
        `../${config.hostname}-private.pem.key`,
      ),
      keys.privateKey,
      { mode: 0o600 },
    ),
  ...
  promises.push(
    writeFile(
      resolve(
        `${config.provisioner.iot.awsCertFilePath}`,
        `../${config.hostname}-certificate-ownership.token`,
      ),
      keys.certificateOwnershipToken,
      { mode: 0o600 },
    ),
  );
  promises.push(
    writeFile(
      resolve(
        `${config.provisioner.iot.awsCertFilePath}`,
        `../${config.hostname}-certificate.id`,
      ),
      keys.certificateId,
      { mode: 0o600 },
    ),
  ...
  const publicKeyText = enc.decode(
    execSync(
      `ssh-keygen -y -f ${resolve(`${config.provisioner.iot.awsCertFilePath}`, `../${config.hostname}-private.pem.key`)}`,
    ),
  );
  await writeFile(
    resolve(
      `${config.provisioner.iot.awsCertFilePath}`,
      `../${config.hostname}-public.pem.key`,
    ),
    publicKeyText,
    { mode: 0o600 },
  );
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      function shadowUpdateRejected(error, response) {
        if (response) {
          console.log(
            "CreateShadow ErrorResponse for " +
              " statusCode=:" +
              response.statusCode +
              " errorCode=:" +
              response.errorCode +
      ...
      console.log(
        `Subscribing to ShadowUpdate Accepted and Rejected topics for ${config.hostname}..`,
      );
...
export async function registerThing(
  identity,
  certId,
    ...
    try {
      let config_builder =
        iot.AwsIotMqttConnectionConfigBuilder.new_mtls_builder_from_path(
          config.provisioner.iot.certFilePath,
          config.provisioner.iot.privateKeyFilePath,
        );
      ...
      connection.on("connect", (session_present) => {
        console.log(
          `Connected to MQTT ${session_present ? "with a new" : "with an existing"} session.`,
        );
        resolve(connection);
      });
